...
class Proyecto(Base):
    ...
    estado = Column(Enum('Planificacion', 'En Progreso', 'Finalizado', 'Cancelado'), default='Planificacion')
...
class Actividad(Base):
    ...
    descripcion = Column(Text)
    prioridad = Column(Enum('Baja', 'Media', 'Alta', 'Urgente'), default='Media')
    fecha_asignacion = Column(Date, nullable=False)
    fecha_limite = Column(Date, nullable=False)
    estado = Column(Enum('Pendiente', 'En Progreso', 'En Revision', 'Completada', 'Cancelada'), default='Pendiente')
    
    # Relaciones
    proyecto = relationship("Proyecto", back_populates="actividades")
    asignaciones = relationship("Asignacion", back_populates="actividad")
    registros_tiempo = relationship("RegistroTiempo", back_populates="actividad")
...
class RegistroTiempo(Base):
    ...
    evidencia_captura_inicio = Column(Integer, ForeignKey('capturas_trabajo.id_captura'))
    evidencia_captura_fin = Column(Integer, ForeignKey('capturas_trabajo.id_captura'))
...
class Entregable(Base):
    ...
    id_actividad = Column(Integer, ForeignKey('actividades.id_actividad'), nullable=False)
    id_empleado = Column(String(5), nullable=False) 
    id_tipo_entregable = Column(Integer, ForeignKey('tipos_entregables.id_tipo_entregable'), nullable=False)
    nombre_archivo = Column(String(255), nullable=False)
    ruta_archivo = Column(String(255), nullable=False)
    fecha_entrega = Column(DateTime, nullable=False)
    version = Column(Integer, default=1)
    estado = Column(Enum('Pendiente Revision', 'En Revision', 'Aprobado', 'Rechazado'), default='Pendiente Revision')
    
    # Relaciones
    actividad = relationship("Actividad", back_populates="entregables")
    tipo_entregable = relationship("TipoEntregable", back_populates="entregables")
    evaluaciones = relationship("EvaluacionCalidad", back_populates="entregable")
...
class EvaluacionCalidad(Base):
    ...
    id_entregable = Column(Integer, ForeignKey('entregables.id_entregable'), nullable=False)
