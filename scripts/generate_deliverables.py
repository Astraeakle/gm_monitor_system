...
def generate_kpi_document():
    ...
    kpi_file = os.path.join(output_dir, f'kpi_document_{datetime.now().strftime("%Y%m%d")}.md')
    ...
    with open(kpi_file, 'w', encoding='utf-8') as f:
        ...
        if 'horas_trabajadas' in df.columns and 'id_empleado' in df.columns:
            horas_por_empleado = df.groupby('id_empleado')['horas_trabajadas'].sum().reset_index()
            horas_por_empleado = horas_por_empleado.sort_values('horas_trabajadas', ascending=False)
            ...
            for _, row in horas_por_empleado.iterrows():
                f.write(f"| {row['id_empleado']} | {row['horas_trabajadas']:.2f} |\n")
            
            f.write("\n")
        
        # KPI: Horas trabajadas por proyecto
        if 'horas_trabajadas' in df.columns and 'id_proyecto' in df.columns:
            horas_por_proyecto = df.groupby(['id_proyecto', 'nombre_proyecto'])['horas_trabajadas'].sum().reset_index()
            horas_por_proyecto = horas_por_proyecto.sort_values('horas_trabajadas', ascending=False)
            ...
            for _, row in horas_por_proyecto.iterrows():
                f.write(f"| {row['id_proyecto']} | {row['nombre_proyecto']} | {row['horas_trabajadas']:.2f} |\n")
        ...
        if 'estado' in df.columns:
            try:
                ...
                f.write("No se pudieron calcular métricas de estado de entregables.\n\n")
        ...
        f.write("- Se recomienda establecer metas de horas productivas por empleado y proyecto.\n")
        f.write("- Implementar revisiones periodicas de la calidad de entregables.\n")
        f.write("- Definir acciones correctivas para mejorar la tasa de aprobacion de entregables.\n")
        f.write("- Crear un sistema de recompensas para los empleados con mayor eficiencia.\n")
